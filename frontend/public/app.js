...
const getUrlData = async(gameDataArr) =>{
    ...
    const fetchPromise = gameImagePathArr.map((imagepath) =>{
        return fetch(`/api/mainscreen/reload/getBinary/${imagepath}`);
    });
...
const ResponseDataAll = async() =>{
    const responseJson = await fetch(`/api/mainscreen/reload/getJson/all`);  // JSONデータを取得しレスポンスを返す（全件取得）
...
const ResponseDataSearch = async(filterValue) =>{
    ...
    const responseJson = await fetch(`/api/mainscreen/reload/getJson/search/${filterValue}`);  
...
const EditGameCard = async() =>{
    ...
    GameCardList.addEventListener('click',async(e) =>{
        ...
        const responseJSON = await fetch(`/api/mainscreen/editCard/getJson/${gameID}`);  // IDをもとにデータベースからデータを取得
        ...
        const responseBinary = await fetch(`/api/mainscreen/reload/getBinary/${gameImagePath}`);  // 画像データを取得
